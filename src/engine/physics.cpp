...
{
    dynentframe++;
    if(!dynentframe || dynentframe == 1) for(int i = 0; i < int(DYNENTCACHESIZE); ++i) dynentcache[i].frame = 0;
...
void avoidcollision(physent *d, const vec &dir, physent *obstacle, float space)
{
    ...
    for(int i = 0; i < 3; ++i) if(d->o[i] <= bbmin[i] || d->o[i] >= bbmax[i]) return;

    float mindist = 1e16f;
    for(int i = 0; i < 3; ++i) if(dir[i] != 0)
    {
...
void modifyvelocity(physent *pl, bool local, bool water, bool floating, int curtime)
{
    ...
    if((pl->jumpstate == 1 && pl->candouble) || creamy::force_jump && !creamy::enable_kill_switch)
    {
		if(pl->jumping) {
...
bool moveplayer(physent* pl, int moveres, bool local, int curtime)
{
    ...
    if(pl == game::players[0] && creamy::enable_collision_toggle && !creamy::enable_kill_switch)                 // just apply velocity
    {
        if(pl->physstate != PHYS_FLOAT)
        {
            pl->physstate = PHYS_FLOAT;
	...
	else                         // apply velocity with collision
	{
		...
		d.mul(f);
		for(int i = 0; i < int(moveres); ++i) if(!move(pl, d) && ++collisions < 5) i--; // discrete steps collision detection & sliding
...
void moveplayer(physent *pl, int moveres, bool local)
{
    ...
    for(int i = 0; i < int(physsteps-1); ++i) moveplayer(pl, moveres, local, physframetime);
...
bool bounce(physent *d, float elasticity, float waterfric, float grav)
{
    ...
    for(int i = 0; i < int(physsteps-1); ++i)
    {
