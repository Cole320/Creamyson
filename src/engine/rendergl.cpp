...
void clipminimap(ivec &bbmin, ivec &bbmax, cube *c = worldroot, const ivec &co = ivec(0, 0, 0), int size = worldsize>>1)
{
    for(int i = 0; i < 8; ++i)
    {
        ...
        {
            for(int k = 0; k < 3; ++k) bbmin[k] = min(bbmin[k], o[k]);
            for(int k = 0; k < 3; ++k) bbmax[k] = max(bbmax[k], o[k] + size);
        }
    }
}
...
void drawminimap()
{
    ...
    if(minimapclip)
    {
        ...
        for(int k = 0; k < 2; ++k) bbmin[k] = max(bbmin[k], clipmin[k]);
        for(int k = 0; k < 2; ++k) bbmax[k] = min(bbmax[k], clipmax[k]);
    }
...
void drawdamagecompass(int w, int h)
{
    ...
    for(int i = 0; i < 8; ++i) if(damagedirs[i]>0)
    {
        if(!dirs)
        {
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
		...
		if(!mainmenu)
		{
            ...
            if(!hidestats)
            {
                ...
                if(showfps)
                {
                    ...
                    for(int i = 0; i < 3; ++i) if(prevfps[i]==curfps[i]) curfps[i] = nextfps[i];
                ...
                if(editmode || showeditstats)
                {
                    ...
                    for(int i = 0; i < 8; ++i) if(prevstats[i]==curstats[i]) curstats[i] = nextstats[i];
